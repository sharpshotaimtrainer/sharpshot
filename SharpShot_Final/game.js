...
expiredIndexes.reverse().forEach(i => {
  ...
  let available = gridPositions.filter(p => !targets.some(t => Math.abs(t.x - p.x) < 1 && Math.abs(t.y - p.y) < 1));
...
canvas.addEventListener('mousedown', () => {
  ...
  for (let i = targets.length - 1; i >= 0; i--) {
    ...
    // Draw the same path the render uses
    ctx.beginPath();
    ctx.arc(t.x, t.y, t.size, 0, Math.PI * 2);
  ...
  if (hit && mode === 'grid') {
    const available = gridPositions.filter(p => !targets.some(t => Math.abs(t.x - p.x) < 1 && Math.abs(t.y - p.y) < 1));
    const newTarget = available[Math.floor(Math.random() * available.length)];
    if (newTarget) targets.push({ ...newTarget, ttl: 2000 });
